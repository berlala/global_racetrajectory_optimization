...
def result_plots(plot_opts: dict,
    ...
    if plot_opts["raceline"]:
        ...
        plt.figure()
        plt.plot(refline[:, 0], refline[:, 1], "k--", linewidth=0.7)  # reference center line of track
        plt.plot(veh_bound1_virt[:, 0], veh_bound1_virt[:, 1], "b", linewidth=0.5) # vehicle safety bound left
        plt.plot(veh_bound2_virt[:, 0], veh_bound2_virt[:, 1], "b", linewidth=0.5) # vehicle safety bound right
        plt.plot(veh_bound1_real[:, 0], veh_bound1_real[:, 1], "c", linewidth=0.5) # vehicle physical bound left
        plt.plot(veh_bound2_real[:, 0], veh_bound2_real[:, 1], "c", linewidth=0.5) # vehicle physical bound right
        plt.plot(bound1_interp[:, 0], bound1_interp[:, 1], "k-", linewidth = 1.5)  # track bound left
        plt.plot(bound2_interp[:, 0], bound2_interp[:, 1], "k-", linewidth = 1.5) # track bound right
